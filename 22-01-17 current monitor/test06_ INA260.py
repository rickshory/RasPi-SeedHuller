...
### try addresses set by jumpers
##for addr in TEST_ADDRS:
##    try:
##        w = read_unsigned_16bit(i2c_bus, addr, MFR_ID)
##        # presume read succeeded
##        print("address {0} returned {1}".format(hex(addr), hex(w)))
##
##    except OSError as e:
##        print("address {0}, error {1}".format(hex(addr), e))
##        # nonexistent address: '[Errno 121] Remote I/O error'
##        # same if I2C is not powered
##        continue
##
...
# gets correct 0x2270
print(hex(read_unsigned_16bit(i2c_bus, INA260_ADDRESS, BUS_VOLTAGE_REG)))
print(hex(read_unsigned_16bit(i2c_bus, INA260_ADDRESS, CURRENT_REG)))
